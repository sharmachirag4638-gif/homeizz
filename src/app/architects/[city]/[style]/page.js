...
import { ARCHITECTURE_STYLES } from '@/lib/styles';
...
const DEFAULT_CITY_IMAGE = 'https://images.unsplash.com/photo-1487958449943-2429e8be8625?w=1200&q=80';

function cityImage(citySlug) {
  return CITY_IMAGES[citySlug] || DEFAULT_CITY_IMAGE;
}

function findArchitectureStyle(slug) {
  return ARCHITECTURE_STYLES.find(style => style.slug === slug.toLowerCase()) || null;
}
...
function pageDescription(style, city) {
  return truncateMeta(
    `Find verified ${style.name.toLowerCase()} architects in ${city.name}, ${city.state}. Compare portfolios, home plans, project fees, and request quotes from local architecture firms on Homeizz.`
  );
}

export async function generateMetadata({ params }) {
  const city = findCity(decodeURIComponent(params.city).toLowerCase());
  const style = findArchitectureStyle(decodeURIComponent(params.style));
  if (!city || !style) return { title: 'Architects not found', robots: { index: false, follow: false } };

  const title = `${style.name} Architects in ${city.name}`;
  const description = pageDescription(style, city);
  const canonical = `/architects/${city.slug}/${style.slug}`;
...
export default async function ArchitectsPage({ params }) {
  ...
  const { data: listings } = await sb
    ...
    .eq('status', 'live')
    .eq('city', city.name)
    .eq('listing_type', 'architecture')
    .ilike('style', style.slug)
    .order('created_at', { ascending: false })
    .limit(30);

  const canonical = `/architects/${city.slug}/${style.slug}`;
  const pageTitle = `${style.name} Architects in ${city.name}`;
  const description = pageDescription(style, city);
    ...
    '@graph': [
      {
        ...
        about: {
          '@type': 'Service',
          name: `${style.name} architecture services`,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Architects', item: absoluteUrl('/browse?type=architecture') },
    ...
    <>
      ...
      <div style={{ background: 'var(--c)', minHeight: '100vh', paddingTop: 64 }}>
        <div style={{ height: 400, position: 'relative', overflow: 'hidden' }}>
          <img src={cityImage(city.slug)} alt={`${city.name} architecture and home design`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '40px 44px' }}>
            <div style={{ maxWidth: 1100, margin: '0 auto', width: '100%' }}>
              <div style={{ fontSize: '.72rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(255,255,255,.68)', marginBottom: 12 }}>
                <Link href="/" style={{ color: 'rgba(255,255,255,.68)' }}>Home</Link> / <Link href="/browse?type=architecture" style={{ color: 'rgba(255,255,255,.68)' }}>Architects</Link> / {city.name}
              </div>
              ...
              <p style={{ color: 'rgba(255,255,255,.76)', fontSize: '.98rem', maxWidth: 640, lineHeight: 1.7 }}>
                Compare verified {style.name.toLowerCase()} architects, house plans, fees, and project portfolios in {city.name}, {city.state}.
              </p>
            </div>
          </div>
        </div>
        ...
        <main style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 44px 80px' }}>
          {!listings || listings.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '64px 24px', background: '#fff', borderRadius: 20, border: '2px dashed var(--borderl)', marginBottom: 40 }}>
              <h2 style={{ fontFamily: 'var(--fd)', color: 'var(--b)', marginBottom: 8 }}>No listed {style.name.toLowerCase()} architects in {city.name} yet</h2>
              <p style={{ color: 'var(--tlt)', fontSize: '.9rem', marginBottom: 24, maxWidth: 560, marginLeft: 'auto', marginRight: 'auto', lineHeight: 1.7 }}>
                We are adding verified architecture firms city by city. You can still browse all Homeizz professionals or list your studio to appear here.
              </p>
              <Link href="/browse?type=architecture" style={{ padding: '12px 28px', background: 'var(--t)', color: '#fff', borderRadius: 50, textDecoration: 'none', fontWeight: 700, fontSize: '.9rem', display: 'inline-block' }}>
              ...
              {listings.map(listing => (
                <Link key={listing.id} href={`/listing/${listing.id}`} style={{ textDecoration: 'none', display: 'block', background: '#fff', borderRadius: 20, overflow: 'hidden', border: '1.5px solid var(--borderl)', boxShadow: 'var(--sh)' }}>
                  <div style={{ height: 220, background: 'linear-gradient(135deg,#F5DDD0,#FBF0E8)', position: 'relative', overflow: 'hidden' }}>
                    {listing.cover_image ? (
                      <img src={listing.cover_image} alt={`${listing.title} architecture project in ${listing.city || city.name}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--tlt)' }}>Architecture project</div>
                    )}
          ...
          <section style={{ background: '#fff', border: '1.5px solid var(--borderl)', borderRadius: 18, padding: '28px 30px', marginBottom: 40 }}>
            <h2 style={{ fontFamily: 'var(--fd)', color: 'var(--b)', fontSize: '1.7rem', marginBottom: 12 }}>Hire the right architect in {city.name}</h2>
            <p style={{ color: 'var(--tm)', fontSize: '.95rem', lineHeight: 1.8, marginBottom: 14 }}>
              A strong {style.name.toLowerCase()} architect helps you plan structure, light, ventilation, materials, budget, and approvals before construction starts. Use this page to compare local portfolios and shortlist professionals who understand {city.name}'s climate, plot sizes, contractor ecosystem, and municipal approval flow.
            </p>
            <p style={{ color: 'var(--tm)', fontSize: '.95rem', lineHeight: 1.8, margin: 0 }}>
              Before you hire, ask for recent drawings, a fee structure, site visit frequency, approval support, and examples of completed homes in or around {city.name}. Homeizz keeps these landing pages focused on local search intent so homeowners can move from discovery to quote comparison quickly.
            </p>
          </section>

          <section style={{ marginBottom: 44 }}>
            <h2 style={{ fontFamily: 'var(--fd)', color: 'var(--b)', marginBottom: 18 }}>Explore architects in other cities</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 12 }}>
              {CITIES.filter(option => option.slug !== city.slug).slice(0, 12).map(option => (
                <Link key={option.slug} href={`/architects/${option.slug}/${style.slug}`} style={{ textDecoration: 'none', display: 'block', borderRadius: 14, overflow: 'hidden', position: 'relative', height: 120 }}>
                  <img src={cityImage(option.slug)} alt={`${style.name} architects in ${option.name}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
