...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${style.charAt(0).toUpperCase()+style.slice(1)} Architects in ${city} | Homeizz`,
    description: `Find verified ${style} architects in ${city}. Compare portfolios, get quotes and build with confidence.`,
  };
}
      ...
      <div style={{background:'var(--c)',minHeight:'100vh',paddingTop:64}}>
          ...
          <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',justifyContent:'flex-end',padding:'40px 44px'}}>
            <div style={{maxWidth:1100,margin:'0 auto',width:'100%'}}>
              <div style={{fontSize:'.72rem',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:'rgba(255,255,255,.6)',marginBottom:12}}>
                <Link href="/" style={{color:'rgba(255,255,255,.6)'}}>Home</Link> › <Link href="/architects" style={{color:'rgba(255,255,255,.6)'}}>Architects</Link> › {city}
              </div>
              <h1 style={{fontFamily:'var(--fd)',color:'#fff',fontSize:'clamp(2rem,5vw,3.5rem)',marginBottom:12,lineHeight:1.1}}>
                {style.charAt(0).toUpperCase()+style.slice(1)} Architects<br/>in {city}
              </h1>
              <p style={{color:'rgba(255,255,255,.7)',fontSize:'.95rem'}}>
                {listings?.length||0} verified architects · {city}
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{background:'#fff',borderBottom:'1px solid var(--borderl)',padding:'16px 44px',overflowX:'auto'}}>
          <div style={{maxWidth:1100,margin:'0 auto',display:'flex',gap:8,flexWrap:'nowrap'}}>
            {STYLES.map(s=>(
              <Link key={s} href={`/architects/${city}/${s}`} style={{padding:'8px 18px',borderRadius:50,border:`1.5px solid ${style===s?'var(--t)':'var(--borderl)'}`,background:style===s?'var(--t)':'#fff',color:style===s?'#fff':'var(--tm)',fontSize:'.82rem',fontWeight:600,textDecoration:'none',whiteSpace:'nowrap'}}>
        ...
        <div style={{maxWidth:1100,margin:'0 auto',padding:'40px 44px 80px'}}>
          {!listings||listings.length===0?(
            <div style={{textAlign:'center',padding:'80px 24px',background:'#fff',borderRadius:20,border:'2px dashed var(--borderl)',marginBottom:40}}>
              ...
              <h3 style={{fontFamily:'var(--fd)',color:'var(--b)',marginBottom:8}}>No {style} architects in {city} yet</h3>
              <p style={{color:'var(--tlt)',fontSize:'.88rem',marginBottom:24}}>We're expanding fast. Meanwhile, try nearby cities or browse all listings.</p>
          ...
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:12}}>
              {CITIES.filter(c=>c.name!==city).map(c=>(
                <Link key={c.name} href={`/architects/${c.name}/${style}`} style={{textDecoration:'none',display:'block',borderRadius:14,overflow:'hidden',position:'relative',height:120}}>
