  ...
  return (
    <div style={{width:'100%',maxWidth:440}}>
      <div style={{textAlign:'center',marginBottom:32}}>
        ...
        <p style={{color:'var(--tlt)',fontSize:'.88rem',marginTop:6}}>{mode==='signup'?'Create your free homeowner account':'Welcome back!'}</p>
      </div>
      <div style={{background:'#fff',borderRadius:20,padding:'32px',border:'1.5px solid var(--borderl)',boxShadow:'var(--shm)'}}>
        <div style={{display:'flex',background:'var(--c)',borderRadius:12,padding:4,marginBottom:24}}>
          {['signin','signup'].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setErr('');setSuccess('');}} style={{flex:1,padding:'10px',border:'none',borderRadius:10,background:mode===m?'#fff':'transparent',color:mode===m?'var(--t)':'var(--tlt)',fontWeight:700,cursor:'pointer',fontSize:'.88rem',transition:'all .2s'}}>
        ...
        <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:16}}>
          {mode==='signup'&&(
            <>
              ...
              <div><label style={labelStyle}>Full Name</label><input required value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Amit Sharma" style={inputStyle}/></div>
              <div><label style={labelStyle}>Phone Number</label><input type="tel" required value={phone} onChange={e=>setPhone(e.target.value)} placeholder="9876543210" style={inputStyle}/></div>
            </>
          )}
          <div><label style={labelStyle}>Email Address</label><input type="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" style={inputStyle}/></div>
          <div><label style={labelStyle}>Password</label><input type="password" minLength={8} required value={password} onChange={e=>setPassword(e.target.value)} placeholder="Min 8 characters" style={inputStyle}/></div>
          <button type="submit" disabled={busy} style={{padding:'14px',background:busy?'var(--borderl)':'var(--t)',color:'#fff',border:'none',borderRadius:12,fontWeight:700,cursor:busy?'not-allowed':'pointer',fontSize:'.95rem',marginTop:4}}>
...
export default function AuthPage() {
  return (
    <div style={{minHeight:'100vh',background:'var(--c)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <Suspense fallback={<div style={{color:'var(--tlt)'}}>Loading...</div>}>
        <AuthForm/>
      </Suspense>
    </div>
  );
