...
export default function ProSignup() {
  ...
  function nextStep(){
    ...
    if(step===5){ if(styles.length===0) return setErr('Select at least one style'); if(projectTypes.length===0) return setErr('Select at least one project type'); }
    setStep(s=>s+1);
  }
  ...
  async function submit(){
    ...
    try {
      ...
      const { error } = await sb.auth.signUp({
        email, password,
        options:{ data:{ full_name:displayName, phone, role:'professional', profile_type:profileType,
          pan:profileType==='individual'?pan:null, company_name:profileType==='firm'?companyName:null,
          gst:profileType==='firm'?gst:null, contact_person:profileType==='firm'?contactPerson:null,
          pro_type:proType, experience, projects_completed:projectsCompleted,
          avg_completion_time:avgCompletionTime, min_budget:minBudget, primary_city:primaryCity,
          other_cities:otherCities, pan_india:panIndia, styles, project_types:projectTypes,
          bio, instagram, website, google_business:googleBusiness, plan:selectedPlan, billing,
      ...
      <div style={{maxWidth:680,margin:'0 auto',padding:'40px 20px 80px'}}>
        ...
        <div style={{background:'#fff',borderRadius:20,padding:'36px 40px',border:'1.5px solid var(--borderl)',boxShadow:'var(--sh)'}}>
          ...
          {step===1&&<div>
            ...
            <p style={{marginBottom:28,fontSize:'.9rem'}}>Start your free 3-month trial — no credit card needed</p>
          ...
          {step===2&&<div>
            ...
            {profileType==='individual'&&<div style={{display:'flex',flexDirection:'column',gap:16}}>
            ...
            {profileType==='firm'&&<div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          {step===6&&<div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              <div>
                <label style={labelStyle}>Bio — Tell homeowners about yourself</label>
                <textarea value={bio} onChange={e=>setBio(e.target.value)} placeholder="I am a Mumbai-based interior designer with 8 years of experience..." style={{...inputStyle,height:100,resize:'vertical'}}/>
              </div>
              ...
              <div>
                ...
                <input type="file" accept="image/*" style={{...inputStyle,padding:'10px 14px',cursor:'pointer'}}/>
              </div>
              <div>
              ...
              <div>
                <label style={labelStyle}>Portfolio Images (up to 5)</label>
                <input type="file" accept="image/*" multiple style={{...inputStyle,padding:'10px 14px',cursor:'pointer'}}/>
                <p style={{fontSize:'.75rem',color:'var(--tlt)',marginTop:6}}>Select up to 5 images of your best work</p>
              </div>
            </div>
          </div>}
          ...
          {step===7&&<div>
            ...
            <p style={{marginBottom:24,fontSize:'.9rem'}}>Start free for 3 months — no credit card needed now</p>
            <div style={{background:'linear-gradient(135deg,#6B7F5E,#4A6040)',borderRadius:12,padding:'14px 20px',marginBottom:24,display:'flex',alignItems:'center',gap:12}}>
              ...
              <div>
                ...
                <div style={{color:'rgba(255,255,255,.8)',fontSize:'.78rem'}}>No payment today. Plan starts after 3 months.</div>
              </div>
            </div>
                ...
                <div key={plan.id} onClick={()=>setSelectedPlan(plan.id)} style={{border:`2px solid ${selectedPlan===plan.id?plan.color:'var(--borderl)'}`,borderRadius:16,padding:'18px 20px',cursor:'pointer',background:selectedPlan===plan.id?`${plan.color}15`:'#fff',position:'relative'}}>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    ...
                    <div style={{textAlign:'right'}}>
                      ...
                      {billing==='annual'&&<div style={{fontSize:'.72rem',color:'#6B7F5E',fontWeight:600}}>₹{plan.annual.toLocaleString()}/year · Save ₹{(plan.monthly*12-plan.annual).toLocaleString()}</div>}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div style={{display:'flex',gap:12,marginTop:32}}>
            ...
            {step<7
              ?<button onClick={nextStep} style={{flex:2,padding:'14px',border:'none',borderRadius:12,background:'var(--t)',color:'#fff',fontWeight:700,cursor:'pointer',fontSize:'.95rem'}}>Continue →</button>
