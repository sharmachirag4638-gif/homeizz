        ...
        <div style={{background:'#fff',borderRadius:20,padding:'36px 40px',border:'1.5px solid var(--borderl)',boxShadow:'var(--sh)'}}>
          ...
          {step===7&&(
            <div>
              ...
              {/* Both billing options shown at once */}
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:24}}>
                {PLANS.map(plan=>(
                  <div key={`${plan.id}-monthly`} onClick={()=>{setSelectedPlan(plan.id);setBilling('monthly');}} style={{border:`2px solid ${selectedPlan===plan.id&&billing==='monthly'?plan.color:'var(--borderl)'}`,borderRadius:16,padding:'16px',cursor:'pointer',background:selectedPlan===plan.id&&billing==='monthly'?`${plan.color}15`:'#fff',position:'relative'}}>
                    {plan.popular&&<div style={{position:'absolute',top:-10,left:'50%',transform:'translateX(-50%)',background:plan.color,color:'#fff',fontSize:'.6rem',fontWeight:700,padding:'2px 10px',borderRadius:50,whiteSpace:'nowrap'}}>POPULAR</div>}
                    <div style={{fontWeight:700,color:'var(--b)',fontSize:'.9rem',marginBottom:2}}>{plan.name}</div>
                    <div style={{fontSize:'.72rem',color:'var(--tlt)',marginBottom:8}}>Monthly</div>
                    <div style={{fontWeight:800,color:plan.color,fontSize:'1.1rem'}}>₹{plan.monthly.toLocaleString()}<span style={{fontSize:'.7rem',fontWeight:500,color:'var(--tlt)'}}>/mo</span></div>
                    <div style={{fontSize:'.72rem',color:'var(--tlt)',marginTop:4}}>{plan.listings} listings · {plan.visibility}</div>
                  </div>
                ))}
                {PLANS.map(plan=>(
                  <div key={`${plan.id}-annual`} onClick={()=>{setSelectedPlan(plan.id);setBilling('annual');}} style={{border:`2px solid ${selectedPlan===plan.id&&billing==='annual'?plan.color:'var(--borderl)'}`,borderRadius:16,padding:'16px',cursor:'pointer',background:selectedPlan===plan.id&&billing==='annual'?`${plan.color}15`:'#fff',position:'relative'}}>
                    <div style={{position:'absolute',top:-10,right:10,background:'#6B7F5E',color:'#fff',fontSize:'.6rem',fontWeight:700,padding:'2px 8px',borderRadius:50}}>2 mo free</div>
                    <div style={{fontWeight:700,color:'var(--b)',fontSize:'.9rem',marginBottom:2}}>{plan.name}</div>
                    <div style={{fontSize:'.72rem',color:'var(--tlt)',marginBottom:8}}>Annual</div>
                    <div style={{fontWeight:800,color:plan.color,fontSize:'1.1rem'}}>₹{plan.annualMonthly.toLocaleString()}<span style={{fontSize:'.7rem',fontWeight:500,color:'var(--tlt)'}}>/mo</span></div>
                    <div style={{fontSize:'.72rem',color:'#6B7F5E',fontWeight:600}}>₹{plan.annual.toLocaleString()}/year</div>
                    <div style={{fontSize:'.72rem',color:'var(--tlt)',marginTop:4}}>{plan.listings} listings · {plan.visibility}</div>
                  </div>
                ))}
              </div>
